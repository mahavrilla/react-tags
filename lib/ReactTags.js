...
class ReactTags extends Component {
  ...
  handleChange(e) {
    const stripStafe = e.target.value.trim().replace(/\W/g, "");
    if (this.props.handleInputChange) {
      this.refs.input.value = stripStafe;
      this.props.handleInputChange(stripStafe);
    }

    var query = stripStafe;
  ...
  handleKeyDown(e) {
    ...
    if (e.keyCode === Keys.ESCAPE && this.state.selectionMode) {
      e.preventDefault();
      e.stopPropagation();
      this.setState({
        selectedIndex: -1,
